...
mode-gated, so the script is not playing and there is nothing for a snapshot to
be wrong about; that is true of the *game* state and not of the camera, which
free roam owns outright while it is enabled. A layer outside `World` is not
asked to `resync`, so a seek or a load left the camera wherever the previous
