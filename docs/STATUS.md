...
|---|---:|---:|---|
| `game/` | 32660 | 134 | engine |
| `hod2lib/` | 15105 | 34 | engine |
| `render/` | 8007 | 29 | render |
| `app/` | 7277 | 29 | app |
| `script/` | 4049 | 25 | engine |
| `ui/` | 3032 | 26 | ui |
...
| Ported outside those ranges | 96 (opcodes, and the classes whose handlers sit elsewhere) |
| Citations checked | 255 ported functions match `functions.tsv` under the same name |
| Spawn classes | **14 of 39** read classes have a module, covering 1292 of 1609 placements |
| Declared `[diverges]` | **126** — where the port knowingly departs from the exe, each with its reason on the spot |
| `[open]` markers in `game/` | **116** — questions the port is honest about not having answered |
...
| | |
|---|---|
| Named functions | 722 in `ghidra/annotations/functions.tsv` |
| Named globals | 371 in `ghidra/annotations/globals.tsv` |
