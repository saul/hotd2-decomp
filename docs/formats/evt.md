...
| `46` | `wait_scripted_actors` | **[proved]** `g_civilians_alive <= op` — the **class-0x10 civilians**. Byte for byte the `43` handler on a different counter — `g_camera_free` included — and all 68 sites in the game pass operand 0, so it is always "wait for the last civilian to leave play". **The `g_camera_free` half makes this an enemy gate as much as a civilian one**: the flag comes from `g_enemy_slots`, so a `46` standing in a room with live zombies is held by the zombies, and the civilian's own VM (`CivilianStepScript`, `FUN_0048B1E0`) has wait bits on `g_enemies_present` and `g_enemies_alive` besides. `playthrough.mjs` refused to shoot at these on the strength of the name and read two of them as hangs — `docs/PLAYER_HANGS.md` item 17 |
