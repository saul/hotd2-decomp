...
| `45` | `wait_script_flag` | **[proved]** `g_script_flags[op] != 0` **and `g_evt_gameplay_live`** (`0x007DCCA4`) — a 256-byte array at `0x009C7200`. It is a **gameplay** gate and effectively nothing else: across all six shipped scripts *every* one of its forty-odd sites names a flag that stage's own `48` never sets. The raisers are actors — `CivilianRunScript` op 0x1C (`0x0048BF2A`), `ZombieStateTargetScriptWithFlag` (`0x0045B1DF`, class 0x30 state 36), `FUN_00433f40` (`0x00433FC1`), `FUN_00473cf0` (`0x00473D76`) and the class-0x44 constructor at `0x0047314A` — so this opcode is *the* way the script waits on one actor finishing. Flags **248** and **254** are the two exceptions and are not stage data at all: they belong to the chapter card and the stage-clear card, which `spawn_simple` (`0A`) places, and 248 is written literally at `0x004348C1`. See [civilians.md](civilians.md#and-how-the-stage-script-finds-out) |
